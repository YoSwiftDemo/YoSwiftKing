...
class YoCalendar: UIView {
    ...
    fileprivate var yearBtn: YoOpthionButton!
    fileprivate var monthBtn: YoOpthionButton!
...
//            make.height.equalTo(KBtnW)
        ...
        yearBtn = YoOpthionButton(frame: CGRect.zero,
                                  row: KShowYearsCount/2,
                                  array: yearArray,
                                  model: self.model)
        yearBtn.isHidden = true
        yearBtn.callBack = { [unowned self](value) in
            if let btn = value as? YoOpthionButton {
...
//            make.top.equalTo(self.snp.top)
            make.top.equalTo(letfView.snp.bottom)
            make.height.equalTo(KBtnW)
...
//        let containerW = (self.frame.width-KTipWidth)*2/5
        ...
        container.snp.makeConstraints { (make) in
            make.left.equalTo(self).offset(KBtnW)
            make.right.equalTo(self).offset(-KBtnW)
            make.top.equalTo(letfView.snp.bottom)
            make.height.equalTo(KBtnW)
//            make.width.equalTo(containerW)
        ...
        preMonthBtn.setImage(UIImage(named:"yo_calendar_left"), for: .normal)
        ...
        monthBtn = YoOpthionButton(frame: CGRect.zero, row: month!-1, array: monthArray, model: self.model)
...
//            make.width.equalTo(containerW/2)
        ...
        nextMonthBtn.setImage(UIImage(named:"yo_calendar_right"), for: .normal)
        ...
        nextMonthBtn.snp.makeConstraints { (make) in
            make.right.equalTo(container.snp.right).offset(-KBtnW/2)
            make.top.equalTo(preMonthBtn.snp.top)
        ...
        backTodayBtn.snp.makeConstraints { (make) in
            ...
            make.right.equalTo(self.snp.right).offset(-KBtnW)
...
//        yearBtn.title = "\(year ?? 0)年"
        monthBtn.title =  "\(year ?? 0)年"+"\(month ?? 0)月"
